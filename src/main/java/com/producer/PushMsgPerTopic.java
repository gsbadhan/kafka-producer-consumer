...
public class PushMsgPerTopic {
    ...
    public static void send(final String topic, List<String> data) {
        for (String msg : data) {
            Future<RecordMetadata> status = producer.send(new ProducerRecord<String, String>(topic, String.valueOf(msg.hashCode()), msg));
            try {
                RecordMetadata metadata = status.get(200, TimeUnit.MILLISECONDS);
                System.out.println("msg [" + data + "] sent to partition[" + metadata.partition() + "] offset[" + metadata.offset() + "]");
    ...
    public static void sendAcrossPartitions(final String topic, int totalPartition, List<String> data) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            Future<RecordMetadata> status = producer.send(new ProducerRecord<String, String>(topic, partition, String.valueOf(data.get(i).hashCode()), data.get(i)));
            try {
                RecordMetadata metadata = status.get(200, TimeUnit.MILLISECONDS);
